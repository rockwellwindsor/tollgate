...
func TestDecide(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "match + active session allow → AllowSilent",
			state: State{GlobalEnabled: true, DefaultAction: "prompt", SessionAllow: true},
			env:   Env{},
			want:  AllowSilent,
		},
		{
			name:  "match + active session deny → DenySilent",
			state: State{GlobalEnabled: true, DefaultAction: "prompt", SessionDeny: true},
			env:   Env{},
			want:  DenySilent,
		},
		{
			name:  "match + TOLLGATE=off → AllowSilent (overrides everything)",
			state: State{GlobalEnabled: true, DefaultAction: "deny", SessionDeny: true},
